...
const successResponse = (bodyObj, code = 200) => ({
    statusCode: code,
    headers: corsHeaders,
...
async function signUpUser(body) {
    ...
    try {
        ...
        return successResponse({ message: "User signed up successfully" });
    } catch (err) {
...
async function signInUser(body) {
    ...
    try {
        ...
        if (data.AuthenticationResult && data.AuthenticationResult.IdToken) {
            return successResponse({ idToken: data.AuthenticationResult.IdToken });
        } else {
...
async function createTable(body) {
    ...
    try {
        ...
        return successResponse({ id: Number(tableItem.id) });
    } catch (err) {
